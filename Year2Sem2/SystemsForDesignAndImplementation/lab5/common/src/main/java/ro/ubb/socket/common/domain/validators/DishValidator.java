...
public class DishValidator implements Validator<Dish>{
    ...
    public void validate(Dish entity) throws ValidatorException {
        ...
        name.filter(x->x.equals("")).ifPresent(s->{throw new ValidatorException("Dish name field cannot be empty!\n");});

        Optional<String> category=Optional.of(entity.getCategory());
        category.filter(x->x.equals("")).ifPresent(s->{throw new ValidatorException("Category field cannot be empty!\n");});

        Optional<Float> price = Optional.of(entity.getPrice());
        price.filter(x -> x <= 0).ifPresent( s -> {throw new ValidatorException("Price must be a greater than 0!\n");});

        Optional<Integer> quantity = Optional.of(entity.getQuantity());
        quantity.filter(x -> x <= 0).ifPresent( s -> {throw new ValidatorException("Quantity must be a positive integer!\n");});
    }
}
