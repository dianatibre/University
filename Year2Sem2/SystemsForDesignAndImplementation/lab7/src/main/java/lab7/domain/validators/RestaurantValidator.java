...
public class RestaurantValidator implements Validator<Restaurant> {
    ...
    public void validate(Restaurant entity) throws ValidatorException {
        ...
        name.filter(x->x.equals("")).ifPresent(s->{throw new ValidatorException("Name field cannot be empty!\n");});

        Optional<Integer> rating=Optional.of(entity.getRating());
        rating.filter(x->x < 0 || x > 5).ifPresent(s->{throw new ValidatorException("Rating should be an integer from 0 to 5!\n");});

        Optional<Integer> capacity = Optional.of(entity.getCapacity());
        capacity.filter(x -> x <= 0).ifPresent( s -> {throw new ValidatorException("Capacity must be a positive integer value!\n");});
